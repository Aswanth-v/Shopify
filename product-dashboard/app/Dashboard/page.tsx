...
import {
  Page, Card, Text, Grid, Badge, Spinner, BlockStack, InlineStack, Divider
...
const COLORS = ['#008060', '#4ba3c3', '#f59e0b', '#ef4444', '#8b5cf6', '#ec4899']
...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (data && data.length > 0) {
        ...
        setInsight('No events yet — visit the product page and click some products first!')
  ...
  async function fetchInsight(data: AnalyticsEvent[]) {
    ...
    try {
      ...
      const analyticsData = { totalEvents: data.length, totalSessions, topProducts }
  ...
  function getTopProducts(data: AnalyticsEvent[]) {
    ...
    data.filter(e => e.product_name).forEach(e => {
  ...
  function getCategoryData(data: AnalyticsEvent[]) {
    const counts: Record<string, number> = {}
    data.filter(e => e.product_category).forEach(e => {
      ...
      <BlockStack gap="500">
        ...
        <Card>
          <BlockStack gap="300">
            <InlineStack align="space-between">
              <Text variant="headingMd" as="h2">🤖 Claude AI Insight</Text>
              <Badge tone="magic">Powered by Claude</Badge>
            </InlineStack>
            {loadingInsight ? (
              <InlineStack gap="200">
                <Spinner size="small" />
                <Text as="p" tone="subdued">Analyzing user behavior...</Text>
              </InlineStack>
            ) : (
        ...
        <Grid>
          <Grid.Cell columnSpan={{ xs: 6, sm: 3, md: 3, lg: 3, xl: 3 }}>
            <Card><BlockStack gap="200">
              <Text as="p" tone="subdued">Total Events</Text>
              <Text variant="heading2xl" as="h3">{events.length}</Text>
            </BlockStack></Card>
          </Grid.Cell>
          <Grid.Cell columnSpan={{ xs: 6, sm: 3, md: 3, lg: 3, xl: 3 }}>
            <Card><BlockStack gap="200">
              <Text as="p" tone="subdued">Unique Sessions</Text>
              <Text variant="heading2xl" as="h3">{totalSessions}</Text>
            </BlockStack></Card>
          </Grid.Cell>
          <Grid.Cell columnSpan={{ xs: 6, sm: 3, md: 3, lg: 3, xl: 3 }}>
            <Card><BlockStack gap="200">
              <Text as="p" tone="subdued">Modal Opens</Text>
              <Text variant="heading2xl" as="h3">{modalOpens}</Text>
            </BlockStack></Card>
          </Grid.Cell>
          <Grid.Cell columnSpan={{ xs: 6, sm: 3, md: 3, lg: 3, xl: 3 }}>
            <Card><BlockStack gap="200">
              <Text as="p" tone="subdued">Products Clicked</Text>
              <Text variant="heading2xl" as="h3">{topProducts.length}</Text>
            </BlockStack></Card>
          </Grid.Cell>
        </Grid>
        ...
        <Grid>
          ...
          <Grid.Cell columnSpan={{ xs: 6, sm: 6, md: 6, lg: 3, xl: 3 }}>
            <Card>
              <BlockStack gap="400">
                <Text variant="headingMd" as="h2">By category</Text>
                <ResponsiveContainer width="100%" height={250}>
                  <PieChart>
          ...
          <Grid.Cell columnSpan={{ xs: 6, sm: 6, md: 6, lg: 3, xl: 3 }}>
            <Card>
              <BlockStack gap="400">
                <Text variant="headingMd" as="h2">Event breakdown</Text>
                <ResponsiveContainer width="100%" height={250}>
        ...
        <Card>
          <BlockStack gap="400">
            ...
            <Divider />
            {events.length === 0 ? (
              <Text as="p" tone="subdued">No events yet — go to the product page and click some products!</Text>
            ) : (
              events.slice(0, 10).map(e => (
                <InlineStack key={e.id} align="space-between">
                  <InlineStack gap="300">
                    <Badge tone={e.event === 'modal_open' ? 'success' : 'info'}>{e.event}</Badge>
                    <Text as="span">{e.product_name?.substring(0, 35) ?? 'General'}</Text>
                  </InlineStack>
